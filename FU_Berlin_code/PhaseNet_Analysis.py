...
class PhaseNet_Analysis (object):
    ...
    def __call__ (self):
        ...
        if self.analysis == True:
            ...
            for i in range (DF_auxiliary_path_file.shape[0]):
                ...
                self.extract_amplitude (p_picks, s_picks)
    ...
    def extract_amplitude (self, p_picks:'pd.DataFrame', s_picks:'pd.DataFrame')-> pd.DataFrame:
        ...
        for i in range (p_picks.shape[0]):
            ...
            stream = self.read_data (p_picks.id.iloc[i])
            ...
            decs = 3
            start_0 = np.trunc(obspy.UTCDateTime.__float__(stream[0].stats.starttime)*10**decs)/(10**decs)
            times_0 = stream[0].times() + start_0

            start_1 = np.trunc(obspy.UTCDateTime.__float__(stream[1].stats.starttime)*10**decs)/(10**decs)
            times_1 = stream[1].times() + start_1

            start_2 = np.trunc(obspy.UTCDateTime.__float__(stream[2].stats.starttime)*10**decs)/(10**decs)
            times_2 = stream[2].times() + start_2
